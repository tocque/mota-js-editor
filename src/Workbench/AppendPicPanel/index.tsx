...
export const AppendPicPanel: FC = () => {
  ...
  const [appendRegisterChecked, setAppendRegisterChecked] = useState(true);
  ...
  const handleChangeColorInput = (e: React.InputEvent<HTMLInputElement>) => {
    const delta = (~~e.target.value) * 30;
  ...
  const handleAppendPicClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    const loc = eToLoc(e);
  ...
  useEditor(() => {
      ...
      reader.onload = function() {
        afterReadFile(reader.result, () => {
          ...
          if (confirm("你确定要快速追加么？")) {
            if (cls == "autotile") {
              handleAppendConfirmClick();
            } else {
              handleQuickAppendConfirmClick();
            }
          }
        });
      };
    ...
    editor.uifunctions.appendMaterialByInfo = function(info) {
      ...
      afterReadFile(img, () => {
        const changeColorInput = document.getElementById("changeColorInput") as HTMLInputElement;
        if (changeColorInput) changeColorInput.value = "0";
    ...
    <div id="left1" className="leftTab" style={{ zIndex: -1, opacity: 0 }}>
      ...
      <div className="leftTabContent">
        <p>
          ...
          <input id="appendRegister" type="checkbox" checked={appendRegisterChecked} onChange={(e) => setAppendRegisterChecked(e.target.checked)} />
        </p>
        <p>
          <small>
        ...
        >
          ...
          <canvas style={{ position: "absolute" }} />
          {/* 用于画出选中文件 */}
          <canvas style={{ position: "absolute", zIndex: 100 }} onClick={handleAppendPicClick} />
          {/* 用于响应鼠标点击 */}
